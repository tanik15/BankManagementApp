...
public class getPendingAccounts extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("action") != null) {
			...
			service.updateAccountRequest(accountId, action);
		}
		List<AccountDetails> accounts = service.getPendingAccounts();
		session.setAttribute("accounts", accounts);
		request.getRequestDispatcher("PendingAccounts.jsp").forward(request, response);
	}
